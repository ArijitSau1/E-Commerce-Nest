...
export class ReviewController {
  ...
  @UseInterceptors(FileInterceptor('image', multerOptions('review')))
  create(
    ...
    if (file) {
      dto.image = `http://localhost:3000/uploads/review/${file.filename}`;
    }
  ...
  @Get('my-review')
  myReviews(@GetUser('id') accountId: string) {
