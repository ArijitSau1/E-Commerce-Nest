...
export class InvoiceService {
  ...
  private buildInvoice(doc: any, order: Order, items: OrderItem[]) {
    doc.fontSize(24).font('Helvetica-Bold').text('ShopEasy', {
      align: 'center',
    });

    doc.fontSize(16).font('Helvetica').text('INVOICE', {
      align: 'center',
    });
    ...
    doc.text(`${order.address.country} - ${order.address.postalCode}`);
    ...
    doc.font('Helvetica').fontSize(12).text('Thank You For Shopping!', {
      align: 'center',
    });
  }

  async generateInvoice(orderId: string, res: any) {
    const { order, items } = await this.getInvoiceData(orderId);
    ...
    this.buildInvoice(doc, order, items);
